		...
		if (request.method === "POST" && url.pathname === "/api/deploy") {
			try {
				...
				if (subData.success && subData.result && subData.result.subdomain) {
					...
					const newSub = `atlas-${Math.random().toString(36).substring(2, 8)}`;
				...
				const workerName = `atlas-panel-${uniqueSuffix}`;
				const dbName = `atlas-db-${uniqueSuffix}`;
				...
				const atlasCode = await githubRes.text();
				const metadata = {
				...
				formData.append("atlas.js", new Blob([atlasCode], { type: "application/javascript+module" }), "atlas.js");
		...
		if (request.method === "POST" && url.pathname === "/api/list-panels") {
			try {
				...
				for (let script of scriptsData.result) {
					if (script.id.startsWith("atlas-panel") || script.id.startsWith("ez-")) {
						panels.push({ name: script.id });
					}
				}
				let latestVersion = "Unknown";
				try {
					const ghRes = await fetch((env.ATLAS_SOURCE_URL || DEFAULT_ATLAS_SOURCE_URL) + "?t=" + Date.now());
					if (ghRes.ok) {
...
<head>
    ...
    <title>Atlas Panel Deployer</title>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center justify-center p-4">
    <div id="mainCard" class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl p-8 relative overflow-hidden z-10">
        ...
        <div class="absolute -right-12 -bottom-12 w-40 h-40 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <h2 class="text-2xl font-black text-gray-900 dark:text-white mb-2">Atlas Auto Deployer</h2>
            <p class="text-sm font-medium text-gray-500 dark:text-zinc-400">نصب خودکار پنل Atlas روی کلودفلر</p>
			<p class="text-sm font-medium text-gray-500 dark:text-zinc-400">استقرار سریع و مدیریت پنل Atlas روی Cloudflare Workers</p>
        </div>
    ...
    try {
        ...
        if (versionText && btnContainer) {
            ...
            if (isLatest) {
                ...
                buttonsHtml += '<button data-name="' + scriptName + '" onclick="updateAtlasPanel(this.dataset.name)" class="flex-1 px-3 py-1.5 bg-indigo-100 hover:bg-indigo-200 text-indigo-700 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 dark:text-indigo-400 font-bold rounded-lg text-xs transition">آپدیت</button>';
            }
            if (devSub) {
            ...
            buttonsHtml += '<button data-name="' + scriptName + '" onclick="deleteAtlasPanel(this.dataset.name)" class="flex-1 px-3 py-1.5 bg-red-100 hover:bg-red-200 text-red-700 dark:bg-red-900/30 dark:hover:bg-red-900/50 dark:text-red-400 font-bold rounded-lg text-xs transition">حذف</button>';
            buttonsHtml += '</div>';
        ...
        async function updateAtlasPanel(scriptName) {
            const token = document.getElementById('updateApiToken').value.trim();
            const statusBox = document.getElementById('update-status');
...
async function deleteAtlasPanel(scriptName) {
    const token = document.getElementById('updateApiToken').value.trim();
    const statusBox = document.getElementById('update-status');
    ...
    try {
        ...
        async function startDeploy() {
            ...
            try {
                ...
                statusText.innerText = 'در حال دریافت پنل Atlas...';
