	...
	async fetch(request, env, ctx) {
		...
		if (request.method === "POST" && url.pathname === "/api/deploy") {
			try {
				...
				const workerName = `atlas-${uniqueSuffix}`;
				...
				formData.append("atlaspanel.js", new Blob([atlasCode], { type: "application/javascript+module" }), "atlaspanel.js");
		...
		if (request.method === "POST" && url.pathname === "/api/list-panels") {
			try {
				...
				for (let script of scriptsData.result) {
					if (script.id.startsWith("atlas")) {
						panels.push({ name: script.id });
					}
				}
				let latestVersion = "Unknown";
				try {
					const ghRes = await fetch("https://raw.githubusercontent.com/leonardo0231/Setad/refs/heads/main/atlaspanel.js?t=" + Date.now());
					if (ghRes.ok) {
    ...
    try {
        ...
        if (versionText && btnContainer) {
            ...
            if (isLatest) {
                ...
                buttonsHtml += '<button data-name="' + scriptName + '" onclick="updateAtlas(this.dataset.name)" class="flex-1 px-3 py-1.5 bg-indigo-100 hover:bg-indigo-200 text-indigo-700 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 dark:text-indigo-400 font-bold rounded-lg text-xs transition">آپدیت</button>';
            }
            if (devSub) {
            ...
            buttonsHtml += '<button data-name="' + scriptName + '" onclick="deleteAtlas(this.dataset.name)" class="flex-1 px-3 py-1.5 bg-red-100 hover:bg-red-200 text-red-700 dark:bg-red-900/30 dark:hover:bg-red-900/50 dark:text-red-400 font-bold rounded-lg text-xs transition">حذف</button>';
            buttonsHtml += '</div>';
        ...
        async function updateAtlas(scriptName) {
            const token = document.getElementById('updateApiToken').value.trim();
            const statusBox = document.getElementById('update-status');
...
async function deleteAtlas(scriptName) {
    const token = document.getElementById('updateApiToken').value.trim();
    const statusBox = document.getElementById('update-status');
