...
const DEFAULT_ATLAS_SOURCE_URL = "https://raw.githubusercontent.com/YOUR_GITHUB_USERNAME/atlas/refs/heads/main/atlas.js";
const DEFAULT_ATLAS_IPS_URL = "https://raw.githubusercontent.com/YOUR_GITHUB_USERNAME/atlas/refs/heads/main/ips.txt";
...
export default {
	async fetch(request, env, ctx) {
		...
		if (Router.isWebSocketUpgrade(request) && url.pathname === "/atlas-ws") {
...
const Router = {
	...
	async handleSubscription(url, env) {
		...
		try {
			...
			if (!user || (user.connection_type && user.connection_type !== atob("dmxlc3M="))) {
	...
	async handlePanel(request, env) {
		...
		const panelHtml = HTML_TEMPLATES.panel
			.replaceAll("__ATLAS_SOURCE_URL__", env.ATLAS_SOURCE_URL || DEFAULT_ATLAS_SOURCE_URL)
			.replaceAll("__ATLAS_IPS_URL__", env.ATLAS_IPS_URL || DEFAULT_ATLAS_IPS_URL);
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				...
				const githubRes = await fetch((env.ATLAS_SOURCE_URL || DEFAULT_ATLAS_SOURCE_URL) + "?t=" + Date.now() + Math.random(), {
					headers: {
				...
				formData.append("atlas.js", new Blob([newCode], { type: "application/javascript+module" }), "atlas.js");
				...
				const errorMsg = err.message + " | در صورت عدم موفقیت، از طریق Wrangler و فایل atlas.js آپدیت دستی را انجام دهید.";
		...
		if (url.pathname === "/api/restart-core" && request.method === "POST") {
			...
			try {
				const githubRes = await fetch((env.ATLAS_SOURCE_URL || DEFAULT_ATLAS_SOURCE_URL) + "?t=" + Date.now(), {
					headers: {
						"Cache-Control": "no-cache, no-store, must-revalidate",
						Pragma: "no-cache",
						Expires: "0",
		...
		if (url.pathname.startsWith("/api/users")) {
			...
			if (isUserAction) {
				...
				if (request.method === "POST") {
					...
					try {
						await env.DB.prepare("INSERT INTO users (username, uuid, limit_gb, expiry_days, limit_req, ips, connection_type, tls, port, fingerprint, ip_limit, used_gb, used_req, created_at, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
							.bind(username, finalUuid, limit_gb ? parseFloat(limit_gb) : null, expiry_days ? parseInt(expiry_days) : null, limit_req ? parseInt(limit_req) : null, ips || null, "vless", tls || "none", port || "443", fingerprint || "chrome", ip_limit ? parseInt(ip_limit) : null, finalUsedGb, finalUsedReq, finalCreatedAt, finalIsActive)
							.run();
						return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" } });
					} catch (error) {
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@" + host + ":80?path=%2Fatlas-ws&security=none&encryption=none&host=" + host + "&fp=" + fp + "&type=ws#" + encodeURIComponent(infoRemark));
		ips.forEach((ip) => {
			ports.forEach((portStr) => {
				const isTlsPort = ["443", "2053", "2083", "2087", "2096", "8443"].includes(portStr);
				const tlsVal = isTlsPort ? "tls" : "none";
				const remark = user.username + " | " + ip + " | " + portStr;
				links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@" + ip + ":" + portStr + "?path=%2Fatlas-ws&security=" + tlsVal + "&encryption=none&insecure=0&host=" + host + "&fp=" + fp + "&type=ws&allowInsecure=0&sni=" + host + "#" + encodeURIComponent(remark));
			});
		});
...
<div id="usage-warning-modal" class="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-orange-500/50 rounded-3xl shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed font-medium">
            درخواست‌های روزانه کلودفلر شما از ۹۰,۰۰۰ عبور کرده است. در صورت عبور از سقف ۱۰۰,۰۰۰ درخواست روزانه، دسترسی به پنل و اتصالات تا ساعت ۳:۳۰ بامداد (به وقت ایران) قطع خواهد شد.
        </p>
...
<div id="update-modal" class="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
        <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
            ...
            <p id="update-modal-text" class="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed font-medium">
                نسخه جدید در دسترس است. اگر آپدیت خودکار جواب نداد، آپدیت دستی را با Wrangler انجام دهید.
            </p>
            <div class="space-y-3">
                ...
                <div class="rounded-xl border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900/50 p-4 text-xs text-gray-600 dark:text-zinc-300 leading-relaxed">
                    اگر آپدیت خودکار انجام نشد، فایل <span class="font-mono font-bold">atlas.js</span> را با Wrangler روی همین Worker دیپلوی کنید.
                </div>
            </div>
        ...
        function getVlessLink(username) {
            ...
            ips.forEach((ip) => {
                ports.forEach((portStr) => {
                    ...
                    const tlsVal = isTlsPort ? 'tls' : 'none';
                    const remark = user.username + ' | ' + ip + ' | ' + portStr;
                    links.push('vle' + 'ss://' + (user.uuid || '') + '@' + ip + ':' + portStr + '?path=%2Fatlas-ws&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
...
async function checkForUpdates(isManual = false) {
    try {
        ...
        const res = await fetch('__ATLAS_SOURCE_URL__?t=' + Date.now(), { signal: controller.signal });
...
async function fetchIpsList() {
    try {
        const response = await fetch('__ATLAS_IPS_URL__');
...
document.addEventListener('DOMContentLoaded', () => {
            if (localStorage.getItem('atlas_path_warned_' + CURRENT_VERSION) !== 'true') {
                const modal = document.getElementById('path-warning-modal');
                const card = modal.querySelector('div');
                modal.classList.remove('opacity-0', 'pointer-events-none');
                modal.classList.add('opacity-100', 'pointer-events-auto');
        ...
        function getVlessLink() {
            ...
            ips.forEach(function(ip, ipIndex) {
                ports.forEach(function(portStr) {
                    ...
                    links.push('vle' + 'ss://' + (u.uuid || '') + '@' + ip + ':' + portStr + '?path=%2Fatlas-ws&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
